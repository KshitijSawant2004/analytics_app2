...
import { fetchAnalytics, getActiveProjectId } from "@/utils/backendClient";
...
export default function FunnelsPage() {
  ...
  useEffect(() => {
    async function loadEventOptionsAndFunnels() {
      try {
        const [eventRows, funnelRows] = await Promise.all([
          fetchAnalytics("/events?groupBy=event_name").catch(() => []),
          fetchAnalytics("/funnels").catch(() => []),
        ]);
  ...
  async function runFunnelAnalysis(config) {
    ...
    try {
      const metrics = await fetchAnalytics("/funnels/analyze", {
        method: "POST",
  ...
  async function saveFunnel() {
    ...
    const path = funnelBuilder.id
      ? `/funnels/${encodeURIComponent(funnelBuilder.id)}`
      : `/funnels`;

    try {
      const saved = await fetchAnalytics(path, {
        method,
        headers: { "Content-Type": "application/json" },
  ...
  async function deleteSavedFunnel(funnelId) {
    try {
      await fetchAnalytics(`/funnels/${encodeURIComponent(funnelId)}`, { method: "DELETE" });

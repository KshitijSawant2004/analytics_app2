...
async function queryJourneyTransitions({ metric, userType, device, country, startDate, endDate, limit, projectId }) {
  const values = [];
...
async function queryJourneyTopPaths({ metric, userType, device, country, startDate, endDate, limit, projectId }) {
  const values = [];
  const whereSql = buildJourneyScopeWhere({ userType, device, country, startDate, endDate, projectId }, values);
  values.push(limit);
...
async function queryJourneyDropoffs({ metric, userType, device, country, startDate, endDate, limit, projectId }) {
  const values = [];
  const whereSql = buildJourneyScopeWhere({ userType, device, country, startDate, endDate, projectId }, values);
  values.push(limit);
...
async function queryJourneyFilterOptions(projectId) {
  const values = [];
  ...
  const result = await pool.query(
    `
      ...
      FROM events
      WHERE user_id IS NOT NULL ${projectFilter}
      LIMIT 5000
    `,
    values
  );
...
router.get("/overview", async (req, res) => {
  try {
    ...
    const [totalEventsResult, totalUsersResult, totalSessionsResult, totalErrorsResult, recentActivityResult] = await Promise.all([
      hasEvents ? pool.query(`SELECT COUNT(*)::int AS value FROM events ${projectFilter}`, projectValues) : Promise.resolve({ rows: [{ value: 0 }] }),
      hasEvents ? pool.query(`SELECT COUNT(DISTINCT user_id)::int AS value FROM events ${projectFilter}`, projectValues) : Promise.resolve({ rows: [{ value: 0 }] }),
      ...
      hasFrontendErrors ? pool.query(`SELECT COUNT(*)::int AS value FROM frontend_errors ${projectFilter}`, projectValues) : Promise.resolve({ rows: [{ value: 0 }] }),
      hasEvents
        ? pool.query(
            `SELECT event_name, user_id, session_id, page, created_at
             FROM events ${projectFilter}
...
router.get("/frontend-errors/summary", async (req, res) => {
  try {
    const projectId = String(req.query.project_id || "").trim() || null;
    const projectFilter = projectId ? `WHERE project_id = $1` : "";
    const projectAndFilter = projectId ? `AND project_id = $1` : "";
    const projectValues = projectId ? [projectId] : [];
    ...
    const [topErrorsResult, frequencyResult, sessionsResult, replaySessionsResult] = await Promise.all([
      pool.query(
        `SELECT message, COUNT(*)::int AS count, COUNT(DISTINCT session_id)::int AS sessions_affected
         FROM frontend_errors ${projectFilter}
         GROUP BY message ORDER BY count DESC LIMIT 10`,
        projectValues
      ),
      pool.query(
        `SELECT DATE(COALESCE(timestamp, created_at))::text AS date, COUNT(*)::int AS count
         FROM frontend_errors ${projectFilter}
         GROUP BY DATE(COALESCE(timestamp, created_at))
         ORDER BY DATE(COALESCE(timestamp, created_at)) DESC LIMIT 14`,
        projectValues
      ),
      pool.query(
        `SELECT COUNT(DISTINCT session_id)::int AS sessions_affected, COUNT(*)::int AS total_errors
         FROM frontend_errors ${projectFilter}`,
        projectValues
      ),
      pool.query(
        `SELECT session_id, user_id, COUNT(*)::int AS error_count, MAX(COALESCE(timestamp, created_at)) AS last_seen
         FROM frontend_errors
         WHERE session_id IS NOT NULL ${projectAndFilter}
         GROUP BY session_id, user_id ORDER BY error_count DESC, last_seen DESC LIMIT 20`,
        projectValues
      ),
    ]);
...
router.get("/user-journeys", async (req, res) => {
  try {
    ...
    const [transitions, topPaths, dropoffs, filterOptions] = await Promise.all([
      queryJourneyTransitions({ metric, userType, device, country, startDate, endDate, limit, projectId }),
      queryJourneyTopPaths({ metric, userType, device, country, startDate, endDate, limit, projectId }),
      queryJourneyDropoffs({ metric, userType, device, country, startDate, endDate, limit, projectId }),
      queryJourneyFilterOptions(projectId),
    ]);
...
router.get("/user-journeys/path-sessions", async (req, res) => {
  try {
    ...
    const values = [];
    if (projectId) values.push(projectId);
    const projectCondition = projectId ? `AND project_id = $${values.length}` : "";
    ...
    const result = await pool.query(
      `
        WITH ordered_events AS (
          ...
          FROM events
          WHERE user_id IS NOT NULL AND session_id IS NOT NULL ${projectCondition}
        )
        SELECT
          user_id,
          session_id,
...
router.post("/funnels/analyze", async (req, res) => {
  try {
    const body = req.body || {};
    // Inject active project_id from query param if not in body
    if (!body.project_id && req.query.project_id) {
      body.project_id = req.query.project_id;
    }
    const metrics = await funnelService.analyzeFunnel(body);
...
// ── Projects CRUD (uses the existing `projects` table) ───────────────────────

// GET /projects — all registered projects with event counts
router.get("/projects", async (_req, res) => {
  try {
    const result = await pool.query(`
      ...
      FROM projects p
      LEFT JOIN (
        SELECT project_id::text, COUNT(*)::int AS event_count, MAX(created_at) AS last_seen
        FROM events
        WHERE project_id IS NOT NULL
        GROUP BY project_id
      ) e ON e.project_id = p.id::text
      ORDER BY COALESCE(e.last_seen, p.created_at) DESC
      LIMIT 100
    `);
...
// POST /projects — create a new project, returns the UUID to use in the SDK
  ...
  try {
    ...
    const result = await pool.query(
      `INSERT INTO projects (id, name, api_key, created_at)
       VALUES (gen_random_uuid(), $1, gen_random_uuid()::text, NOW())
       RETURNING id::text AS project_id, name, api_key`,
      [name]
    );
...
// PUT /projects/:projectId/name — rename a project
router.put("/projects/:projectId/name", async (req, res) => {
  try {
    ...
    const result = await pool.query(
      `UPDATE projects SET name = $1 WHERE id = $2::uuid RETURNING id::text AS project_id, name`,
      [name, projectId]
    );
    ...
    console.error("Project rename error:", error.message);
    return res.status(500).json({ error: "Failed to rename project" });
  }
});

// DELETE /projects/:projectId — remove a project (does not delete event data)

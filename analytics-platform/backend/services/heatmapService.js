...
let ensureHeatmapTablesPromise = null;
...
async function ensureHeatmapTables() {
  if (ensureHeatmapTablesPromise) return ensureHeatmapTablesPromise;

  ensureHeatmapTablesPromise = (async () => {
    await pool.query(`
    ...
    // Add project_id to all heatmap tables for multi-site isolation
  ...
  try {
    await ensureHeatmapTablesPromise;
  } catch (error) {
    ensureHeatmapTablesPromise = null;
...
async function getClickHeatmap({
  ...
  const values = [page_url, start_date, end_date, safeBucketSize, device_type, limit, project_id];
...
async function getScrollHeatmap({ page_url, start_date, end_date, project_id = null }) {
  await ensureHeatmapTables();
...
async function getLatestPageSnapshot({ page_url, start_date, end_date, project_id = null, device_type = null }) {
  await ensureHeatmapTables();
...
async function getHeatmapStats({ page_url, start_date, end_date, project_id = null }) {
  await ensureHeatmapTables();

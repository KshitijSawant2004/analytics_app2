...
async function analyzeFunnel({ steps, mode, analysis_mode, window_hours, project_id }) {
  ...
  const queryValues = [stepNames];
  const projectCondition = project_id ? `AND project_id = $2` : "";
  if (project_id) queryValues.push(project_id);

  const result = await pool.query(
    `
      SELECT user_id, session_id, event_name, created_at
      FROM events
      WHERE event_name = ANY($1::text[]) ${projectCondition}

...
async function ensureTables() {
  ...
  await pool.query(`CREATE INDEX IF NOT EXISTS idx_session_recordings_project_id ON session_recordings(project_id)`);
  ...
  await pool.query(`CREATE INDEX IF NOT EXISTS idx_frontend_errors_project_id ON frontend_errors(project_id)`);
...
async function insertSessionRecordingBatch({
  ...
  const query = `
    ...
    VALUES ($1, $2, $3, $4, $5::jsonb, $6, $7, $8, $9, $10, $11::timestamptz, $12::timestamptz, $13::timestamptz, $14, $15)
  `;
...
async function insertFrontendError({ user_id, session_id, project_id, message, stack, page, timestamp }) {
  await ensureTables();

  const query = `
    INSERT INTO frontend_errors (id, user_id, session_id, project_id, message, stack, page, timestamp)
    VALUES ($1, $2, $3, $4, $5, $6, $7, $8::timestamptz)
  `;

  const values = [uuidv4(), String(user_id || ""), String(session_id || ""), project_id || null, String(message || ""), stack || null, page || null, timestamp || null];
  await pool.query(query, values);
}
...
async function getSessionSummaries(limit = 100, projectId = null) {
  await ensureTables();

  const projectCondition = projectId ? `WHERE project_id = $2` : "";
  const queryValues = projectId ? [limit, projectId] : [limit];
...
async function deleteAllSessionRecordings() {
  ...
  try {
    ...
    const errorsDelete = await client.query("DELETE FROM frontend_errors");
    const recordingsDelete = await client.query("DELETE FROM session_recordings");
...
async function deleteSessionRecording(sessionId, userId) {
  ...
  try {
    ...
    if (normalizedUserId) {
      errorsDelete = await client.query(
        "DELETE FROM frontend_errors WHERE session_id = $1 AND user_id = $2",
        [normalizedSessionId, normalizedUserId]
      );
      recordingsDelete = await client.query(
        "DELETE FROM session_recordings WHERE session_id = $1 AND user_id = $2",
        [normalizedSessionId, normalizedUserId]
      );
    } else {
      errorsDelete = await client.query("DELETE FROM frontend_errors WHERE session_id = $1", [normalizedSessionId]);
      recordingsDelete = await client.query("DELETE FROM session_recordings WHERE session_id = $1", [normalizedSessionId]);
    }
